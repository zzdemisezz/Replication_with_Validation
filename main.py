...
def train_pipeline(data, sens_feature, backend, debiased=True):
    ...
    x_train, x_val, x_test, y_train, y_val, y_test, numvars, categorical = data
    ...
    clf = AdversarialDebiasing([sens_feature], adversary_loss_weight=1, num_epochs=n_epoch, batch_size=256,
                               random_state=29, debias=debiased)
    start = time.time()
    ...
    acc = accuracy_score(y_val[classification], y_pred)
    # deo = DifferenceEqualOpportunity(y_pred, y_train, sens_feature, classification, 1, 0, [0, 1])
    dao = DifferenceAverageOdds(y_pred, y_val, sens_feature, classification, 1, 0, [0, 1])
